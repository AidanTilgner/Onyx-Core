...
export const generateExistingActions = async () => {
  const existingActions = await getExistingActions();

  const formatted = formatJSON(JSON.stringify(existingActions, null, 2));
  writeFileSync(
    `${pathToLocal}/existing_actions.autogenerated.json`,
    formatted
  );
};
...
export const generateExistingActionsWithoutResponse = () => {
  const existingActions = getExistingActionsWithoutResponse();

  const formatted = formatJSON(JSON.stringify(existingActions, null, 2));
  writeFileSync(
    `${pathToLocal}/existing_actions_without_response.autogenerated.json`,
    formatted
  );
};
...
export const addResponseToAction = (act: string, response: string) => {
  ...
  const formatted = formatJSON(JSON.stringify(copyJSON, null, 2));
